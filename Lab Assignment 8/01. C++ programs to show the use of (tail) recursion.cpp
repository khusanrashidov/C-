...
int Fibonacci(int n, int a = 0, int b = 1) //function with default values in parameters
{
    if (n == 0) //if the input is 0 then the output is 0
        return a;
    if (n == 1) //if the input is 1 then the output is 1
        return b;
    return Fibonacci(n - 1, b, a + b); //Fibonacci formula
}
...
int main()
{
    ...
    cout << "3. Display Fibonacci Series Using Recursion." << endl;
